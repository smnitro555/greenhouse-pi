...
class Greenhouse:
    ...
    def initialize(self):
        ...
        for key in self.rf_transmit_keys.keys():
            # Setup Button
            GPIO.setup(
                self.rf_transmit_keys[key]["ButtonGPIO"],
                GPIO.IN,
        ...
        GPIO.add_event_detect(
            self.rf_transmit_keys["VentFan"]["ButtonGPIO"],
            GPIO.FALLING,
            callback=self.vent_fan_callback,
            bouncetime=300,
        )
        GPIO.add_event_detect(
            self.rf_transmit_keys["Heater"]["ButtonGPIO"],
            GPIO.FALLING,
            callback=self.heater_callback,
            bouncetime=300,
        )
        GPIO.add_event_detect(
            self.rf_transmit_keys["GrowLight"]["ButtonGPIO"],
            GPIO.FALLING,
            callback=self.grow_light_callback,
            bouncetime=300,
        )
        GPIO.add_event_detect(
            self.rf_transmit_keys["StandFan"]["ButtonGPIO"],
            GPIO.FALLING,
            callback=self.stand_fan_callback,
            bouncetime=300,
        )

    def switch_off(self, device_name):
        os.system(
            "../../rfoutlet/codesend %i" % self.rf_transmit_keys[device_name]["SendOff"]
        )
    ...
    def switch_on(self, device_name, update_val=time.time()):
        os.system(
            "../../rfoutlet/codesend %i" % self.rf_transmit_keys[device_name]["SendOn"]
        )
    ...
    def write_html(self):
        with open("../index.html", "w") as html_file:
            ...
            html_file.write(
                "<p>Date Last Updated: %s</p>" % self.now.strftime("%H:%M:%S")
            )
    ...
    def main(self):
        ...
        while True:
            ...
            if (
                (self.switch_status["Heater"] is not False)
                and (self.switch_status["Heater"] > 0)
                and (time.time() - self.switch_status["Heater"] > SWITCH_TIMEOUT)
            ):
                self.switch_off("Heater")
            if (
                (self.switch_status["VentFan"] is not False)
                and (self.switch_status["VentFan"] > 0)
                and (time.time() - self.switch_status["VentFan"] > SWITCH_TIMEOUT)
            ):
                self.switch_off("VentFan")
